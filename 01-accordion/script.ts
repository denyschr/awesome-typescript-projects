class Accordion {
  ...
  constructor(accordion: HTMLElement) {
    ...
    this._tabs = this._container.querySelectorAll<HTMLElement>(
      '[data-accordion-tab]'
    );
  ...
  init(): void {
    ...
    const activeTabs = Array.from(this._tabs).filter(activeTab =>
      activeTab.hasAttribute('data-accordion-tab-active')
    );
